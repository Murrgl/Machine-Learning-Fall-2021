""""
Sends the OpenFace data via a socket.
...
# Set up sending data/receiving data.
class sendDataFrame(abstractmethod(ABC)):
    """Initializes the sockets for use in Unity or Livelink?"""
    ...
    def __init__(self, module_id="module", loglevel="INFO",
        ...
        # Gets filepath of the file calling this script.
        frame = inspect.stack()[1]
        module = inspect.getmodule(frame[0])
        file_path = Path(module.sys.argv[0]).absolute()
        # Uses module name in logfile
        logfile = Path(file_path.parent, "logging", "logging_%s.log" % module_id)
        # Makes the log file if it doesn't exist yet.
        ...
        # Sets up socket only if a port is given
        if pub_port:
            logging.info("Publisher port is specified")
            self.pub_socket = createSocket(self.zeromq_context(pub_ip, pub_port, zmq.PUB, pub_bind),
                                           pub_key, "pub.csv")
            logging.info("Publisher socket set-up complete")
        else:
            logging.info("pub_port not specified, not setting-up publisher")

        # Sets up subscriber socket only if a port is given
        if sub_port:
            ...
            self.sub_socket = createSocket(self.zeromq_context(sub_ip, sub_port, zmq.SUB, sub_bind),
                                           sub_key, "sub.csv")
        ...
        # Sets up dealer socket only if a port is given
        if deal_port:
        ...
        # Sets up  dealer socket only if a port is given; TODO better solution for multiple same sockets
        if deal2_port:
        ...
        # Sets up  router socket only if a port is given
        if rout_port:
...
class createSocket:
    ...
    def __init__(self, socket, key='', csv_filename="test.csv"):
        ...
        self.pub_timestamp_old = findCurrentTime()
    ...
    async def sub(self, raw=False):
        ...
        if timestamp:
            ...
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                ...
                self.sub_time_received = findCurrentTime()
    ...
    def write_to_csv(self, data):
        ...
        with open(self.csv_location, 'a') as file:
            writer = csv.writer(file, delimiter=',')
            # Adds frame no to beginning of data list
...
def findCurrentTime():
    """Return time in 100 nanoseconds"""
